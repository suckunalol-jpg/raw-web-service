...
function hasPerm(member) {
  if (!ALLOWED_ROLES.length) { console.warn("[WARN] ALLOWED_ROLES not set — denying all"); return false; }
  return member.roles.cache.some(r => ALLOWED_ROLES.includes(r.id));
}

function ah() { return {"Content-Type":"application/json","x-admin-password":UPLOAD_PASSWORD}; }
...
client.on("interactionCreate", async interaction => {
  ...
  if (cmd === "upload") {
    ...
    let content = inline || "";
    if (att) { try { content = await (await fetch(att.url)).text(); } catch { return interaction.editReply({embeds:[new EmbedBuilder().setColor(C.err).setDescription("❌ Failed to fetch attachment.")]}); } }
  ...
  if (cmd === "listscripts") {
    ...
    const rows = list.map((s,i)=>`\`${String(i+1).padStart(2,"0")}\` **${s.name}** — ${s.lines} lines · ${(s.size/1024).toFixed(1)}KB · ▶ ${s.executions} runs${s.description?`\n> ${s.description}`:""}`).join("\n\n");
  ...
  if (cmd === "resetexecs") {
    ...
    const data = await res.json();
    return interaction.editReply({embeds:[new EmbedBuilder().setColor(data.success?C.ok:C.err).setDescription(data.success?`✅ Counter reset for \`${name}\`.`:"❌ "+data.error)]});
  }
...
client.once("ready", () => console.log(`🛡️ PubArmour bot: ${client.user.tag}`));
